...
export function buildRuntimeConfigOptionPairs(
  ...
  const shouldEmitAdvertisedOption = (key: string): boolean =>
    advertisedKeys.size === 0 ||
    resolveRuntimeConfigOptionAliases(key).some((alias) =>
      advertisedKeys.has(normalizeLowercaseStringOrEmpty(alias)),
    );
  if (normalized.model && shouldEmitAdvertisedOption("model")) {
    pairs.set(resolveRuntimeConfigOptionKey("model", advertisedConfigOptionKeys), normalized.model);
  }
  if (normalized.thinking && shouldEmitAdvertisedOption("thinking")) {
    pairs.set(
      resolveRuntimeConfigOptionKey("thinking", advertisedConfigOptionKeys),
      normalized.thinking,
    );
  }
  if (normalized.permissionProfile && shouldEmitAdvertisedOption("approval_policy")) {
    pairs.set(
...
function resolveRuntimeConfigOptionKey(
  key: string,
  advertisedConfigOptionKeys?: readonly string[],
): string {
