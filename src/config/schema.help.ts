...
export const FIELD_HELP: Record<string, string> = {
  ...
  "agents.list[].subagents.allowUnsandboxedTargets":
    "Allow a sandboxed requester to spawn these unsandboxed target agent ids. Targets must also pass allowAgents. Empty or omitted keeps the current reject. Use this only for host-ro routers (sandbox.mode=off, tools.exec.security=deny, workspaceAccess=ro). allowAgents alone cannot escape the sandbox.",
  "agents.defaults.subagents.allowUnsandboxedTargets":
    "Default unsandboxed-target exception list used when the requester agent omits subagents.allowUnsandboxedTargets. Targets must still pass allowAgents. Empty or omitted keeps the reject. Use this only for host-ro routers, not host-exec shells.",
